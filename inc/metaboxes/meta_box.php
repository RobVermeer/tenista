...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'email':
		default:
			echo '<input placeholder="' . $placeholder . '" type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		case 'url':
			echo '<input placeholder="' . $placeholder . '" type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_url( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		case 'number':
			echo '<input placeholder="' . $placeholder . '" type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . intval( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		// textarea
		case 'textarea':
			echo '<textarea placeholder="' . $placeholder . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="55" rows="4">' . esc_textarea( $meta ) . '</textarea>
					<br />' . $desc;
		break;
		// editor
		case 'editor':
		...
		// date
		case 'date':
			echo '<input type="text" class="datepicker" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . ($meta ? date('d-m-Y', $meta) : '') . '" size="30" />
					<br />' . $desc;
		break;
		// slider
		case 'slider':
...
class Custom_Add_Meta_Box {
    ...
    public function __construct( $id, $title, $fields, $page, $context = 'normal' ) {
		$this->id = $id;
		$this->title = $title;
		$this->fields = $fields;
		$this->page = $page;
	...
	function admin_head() {
		global $pagenow;
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) && ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) ) ) {
	...
	function save_box( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			elseif( in_array( $field['type'], array( 'date' ) ) ) {
				$date = strtotime($_POST[$field['id']]);
				update_post_meta( $post_id, $field['id'], $date );
			}
			else {
				// save the rest
				$new = false;
